...
Dane zawierają informację o lądowaniach różnych samolotów, reprezentujących 
różne linie lotnicze na różnych lotniskach na świecie.

W strumieniu pojawiają się zdarzenia zgodne ze schematem `FlightEvent`:

```
create json schema 
FlightEvent(aircraft string, airline string, airport string, flight string, 
            clouds string, wind_dir string, wind_str double, 
            ets string, its string);
```

Każde zdarzenie związane z jest z faktem zarejestrowania 
lądowania pojedynczej maszyny na danym lotnisku przy określonych warunkach pogodowych.
...
Atrybuty w każdym zdarzeniu zgodnym ze schematem `FlightEvent` 
...
* `flight`- kod lotu
* `clouds`- kod opisujący poziom zachmurzenia podczas lądowania 
   (zgodny ze standardem `METAR`)
...
Poszczególne wartości atrybutu `clouds` zgodne są ze standardem `METAR` 
i zostały wylosowane z rozkładu innego niż jednorodny:

* `SKC` (15.0% szansy) - No cloud/Sky clear, czyste niebo
* `NSC` (15.0% szansy) - No (nil) significant cloud, niebo częściowo zachmurzone, 
   ale powyżej bezpiecznego pułapu (1500 m)
...
```aidl
@name('answer') SELECT aircraft, airline, airport, flight, clouds, 
     wind_dir, wind_str, ets, its 
...
- `distinct_aircrafts` - liczbę różnych typów maszyn zarejestrowanych w 
...
## Zadanie 3
Znajduj przypadki, w których lądowanie przebiegało przy wietrze dwa razy 
silniejszym niż średni w ciągu 100 ostatnich lądowań dla danego portu lotniczego.
...
- `airport` - nazwę (kod) lotniska
- `flight` - kod lotu
- `wind_str` - siła wiatru podczas lądowania
- `avg_wind_str` - średnią siłę wiatru dla danego lotniska z ostatnich 100 lądowań.
...
## Zadanie 5
Znajduj przypadki wystąpienia serii trwającej co najmniej 30 sekund 
(czas od rejestracji pierwszego do ostatniego zdarzenia) i zawierającej 
co najmniej 5 lądowań na lotnisku `OIAW`, 
w trakcie których prędkość wiatru nie przekraczała `90km/h`. 
Wypisz nazwy linii lotniczych z trzech pierwszych lądowań w serii.
...
niepokojącą sytuację a niebo nadal pozostaje w pełni zachmurzone (`OVC`) (B). 
Ostatecznie niebezpieczna sytuacja kończy się lądowaniem (C) przy prędkości 
